...
function propertyToImage(property: NotionProperty | undefined) {
  if (!property) return "";
  const fileUrl = property.files?.map((file) => file.external?.url || file.file?.url).find(Boolean);
...
function parsePage(page: NotionPage): ContentPost | null {
  ...
  return { id: page.id, title, summary, image, url, contentType, date };
}

async function fetchContentPostsFromNotion() {
...
export default async function handler(req: ApiRequest, res: ApiResponse) {
  ...
  try {
    const contentTypeValue = req.query?.contentType;
    const limitValue = req.query?.limit;
    const contentType = typeof contentTypeValue === "string" ? contentTypeValue : "정보성";
    const limit = Math.min(
      12,
      Math.max(1, Number(typeof limitValue === "string" ? limitValue : "3") || 3)
    );

    const posts = await fetchContentPostsFromNotion();
    const filtered = posts.filter((post) => post.contentType.includes(contentType)).slice(0, limit);

    return res.status(200).json({ posts: filtered, count: filtered.length });
  } catch (error) {
    console.error("content-posts api failed", error);
    return res.status(200).json({ posts: [], count: 0, error: String(error) });
  }
}
