// 작업일지(/work/:slug) 슬러그 생성 로직
// 클라이언트(client/src/lib/workSlug.ts), 프리렌더 스크립트, api/work-og.ts, api/sitemap.ts가
// 모두 같은 슬러그를 만들어야 하므로 공용 모듈로 둔다.

export type WorkSlugPost = {
  title: string;
  area?: string;
  date: string;
};
...
export function getWorkSlug(post: WorkSlugPost) {
