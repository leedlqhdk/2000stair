...
const serviceAreas = [
  { name: "신둔면", slug: "sindun", summary: "공동현관 · 계단 정기관리", position: "top-[25%] left-[32%]" },
  { name: "마장면", slug: "majang", summary: "빌라 · 상가 공용부 관리", position: "top-[52%] left-[24%]" },
  { name: "부발읍", slug: "bubal", summary: "상가 · 소형 건물 관리", position: "top-[46%] left-[74%]" },
  { name: "증포동", slug: "downtown", summary: "이천 시내권 계단 · 복도 관리", position: "top-[34%] left-[49%]" },
  { name: "중리동", slug: "downtown", summary: "시내권 상가 · 공용공간 관리", position: "top-[47%] left-[43%]" },
  { name: "관고동", slug: "downtown", summary: "건물 공용부 · 유리 관리", position: "top-[37%] left-[52%]" },
  { name: "대월면", slug: "daewol", summary: "빌라 계단 · 정기관리", position: "top-[68%] left-[65%]", featured: true },
];
...
const reviews = [
  ...
  { source: "숨고 리뷰", sourceClass: "text-[#6b4eff]", area: "전문 서비스 리뷰", text: "계단·화장실·건물 내부 청소 후기를 확인해보세요.", href: "https://soomgo.com/profile/users/3729049" },
  {
    source: "네이버 플레이스",
...
const areaLabels: Record<string, string> = { majang: "마장면", daewol: "대월면", sindun: "신둔면", downtown: "시내권", gonjiam: "곤지암" };
...
export default function Blog() {
  ...
  const recentPosts = notionRecentPosts && notionRecentPosts.length > 0 ? notionRecentPosts : fallbackRecentPosts;
    ...
    <div className="min-h-screen bg-gradient-to-b from-white via-blue-50/20 to-white">
      ...
      <main>
        <section className="container max-w-6xl pt-24 pb-14 md:pt-32 md:pb-20">
          <motion.div className="mb-10 md:mb-14 text-center" initial={{ opacity: 0, y: 34 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.65 }}>
            <Link href="/">
            ...
            <p className="text-xs md:text-sm font-bold tracking-[0.35em] text-primary mb-4">FIELD ARCHIVE</p>
            <h1 className="text-3xl md:text-4xl font-extrabold leading-[1.12] text-foreground mb-4">관리 지역</h1>
            <p className="text-base md:text-lg text-muted-foreground leading-relaxed max-w-2xl mx-auto">이천 지역 공용공간을 깨끔하게 관리합니다. 원하는 지역을 선택해 관리 현장을 확인하세요.</p>
          </motion.div>

          <motion.div className="mx-auto mb-12 w-full max-w-[430px] overflow-hidden rounded-[1.75rem] border border-blue-100 bg-white shadow-sm md:mb-16 md:max-w-none" initial={{ opacity: 0, y: 28 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.65, delay: 0.08 }}>
            <div className="relative bg-white px-4 py-7 md:bg-gradient-to-b md:from-blue-50/50 md:via-white md:to-blue-50/30 md:px-10 md:py-12">
              <div className="mb-7 flex flex-col gap-2 md:mb-8 md:flex-row md:items-end md:justify-between">
                <div>
                  <p className="mb-3 text-sm font-extrabold text-primary">홈 · 방문지역</p>
                  <h2 className="text-[2rem] font-extrabold leading-tight tracking-normal text-foreground md:text-3xl">이천 생활권<br />직접 관리합니다</h2>
                  <p className="mt-4 text-base font-medium leading-relaxed text-muted-foreground md:max-w-xl">홈페이지에 안내된 청소 가능지역을 기준으로 주소와 사진을 확인한 뒤 무료 방문 견적을 안내드립니다.</p>
                </div>
                <p className="hidden md:block text-sm font-medium text-primary">지역을 누르면 지도와 작업현장 버튼이 함께 반응합니다</p>
              </div>
          ...
          <section className="mb-12 md:mb-16">
            <div className="mb-5 flex items-center justify-between gap-4">
              <div>
                <h2 className="text-xl md:text-2xl font-extrabold text-foreground">고객님들의 실제 후기</h2>
                <p className="mt-1 text-sm text-muted-foreground">실제 관리 경험을 바탕으로 남겨주신 후기입니다.</p>
              </div>
            </div>
            <div className="grid gap-4 md:grid-cols-3">
              {reviews.map((review) => (
                <a key={review.source} href={review.href} target="_blank" rel="noopener noreferrer" className="block rounded-[1.5rem] border border-blue-100 bg-white p-5 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-lg">
                  ...
                  <p className={`text-sm font-bold mb-2 ${review.sourceClass}`}>{review.source}</p>
                  ...
                  <div className="mt-4 flex items-center justify-between gap-3">
                    ...
                    <span className="inline-flex items-center text-xs font-bold text-primary">후기 보기<ArrowRight className="ml-1 h-3.5 w-3.5" /></span>
                  </div>
                </a>
              ))}
            </div>
          ...
          <div className="mb-6 flex items-end justify-between gap-4">
            <div>
              <h2 className="text-xl md:text-2xl font-extrabold text-foreground">최근 관리 현장</h2>
          ...
          {isLoading ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {Array.from({ length: 3 }).map((_, i) => (
                <div key={i} className="overflow-hidden rounded-[1.75rem] border border-blue-100 bg-white">
                  <Skeleton className="h-64 w-full" />
                  <div className="p-5"><Skeleton className="h-6 w-3/4 mb-3" /><Skeleton className="h-4 w-1/3" /></div>
                </div>
              ))}
            </div>
          ) : (
            <motion.div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6" initial="hidden" whileInView="visible" viewport={{ once: true }} variants={{ hidden: {}, visible: { transition: { staggerChildren: 0.08 } } }}>
              {recentPosts.slice(0, 6).map((post, index) => {
                ...
                return (
                  <motion.div key={`${areaSlug}-${post.title}-${index}`} className="h-full" variants={{ hidden: { opacity: 0, y: 28 }, visible: { opacity: 1, y: 0, transition: { duration: 0.45 } } }}>
                      ...
                      <article className="group overflow-hidden rounded-[1.75rem] border border-blue-100 bg-white shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-500 cursor-pointer h-full">
                        <div className="relative h-72 overflow-hidden bg-blue-50">
                          ...
                          <div className="absolute left-5 bottom-5 right-5">
                            <div className="flex items-center gap-1 text-xs text-white/80 mb-2"><CalendarDays className="w-3 h-3" /><span>{post.date}</span></div>
                            <h2 className="text-white text-xl font-extrabold leading-snug line-clamp-2">{post.title}</h2>
                          </div>
                        </div>
                        <div className="p-5"><p className="text-sm font-bold text-primary mb-2">{areaName}</p><p className="text-sm text-muted-foreground line-clamp-2">{description}</p></div>
                      </article>
                    </Link>
                  </motion.div>
                );
...
function ResponsiveAreaSelector() {
  const [selectedAreaName, setSelectedAreaName] = useState(serviceAreas.find((area) => area.featured)?.name ?? serviceAreas[0].name);
  ...
  return (
    <div className="mx-auto space-y-5 md:max-w-[920px]">
      <div>
        ...
        <div className="mt-3 flex items-center gap-2 text-sm font-extrabold text-slate-600 md:mt-5 md:text-lg"><MapPin className="h-4 w-4 shrink-0 text-primary md:h-6 md:w-6" />아래 동네를 누르면 지도에 위치가 표시됩니다</div>
        <div className="-mx-1 mt-3 flex gap-2 overflow-x-auto px-1 pb-1 md:mt-5 md:flex-wrap md:gap-4 md:overflow-visible">
          {serviceAreas.map((area) => (
            <button key={area.name} type="button" onClick={() => setSelectedAreaName(area.name)} className={`shrink-0 rounded-full border px-5 py-3 text-sm font-extrabold shadow-sm transition hover:-translate-y-0.5 hover:border-primary/50 md:min-w-32 md:px-7 md:py-4 md:text-lg ${area.name === selectedArea.name ? "border-primary bg-primary text-white shadow-blue-900/15" : "border-blue-100 bg-white text-foreground hover:bg-blue-50"}`}>
              {area.name}
            </button>
          ))}
        </div>
