...
const faqs = [
{
question: "호법면 어느 지역까지 가능한가요?",
answer: "안평리, 매곡리, 주박리 등 호법면 주요 생활권은 상담 가능합니다. 사진과 주소를 보내주시면 방문 가능 여부를 먼저 확인합니다.",
},
{
question: "작업 기록이 아직 없어도 상담 가능한가요?",
answer: "가능합니다. 호법면 작업 사진은 순차적으로 정리 중이며, 견적은 현재 건물 상태와 관리 범위를 기준으로 안내드립니다.",
},
{
...
export default function HobeopAreaPage() {
const { posts } = useAreaPosts("hobeop", fallbackPosts);
...
<AreaIntro
headline="호법면 계단청소와 공용공간 관리를 준비하고 있습니다"
description="호법면 빌라·원룸·상가 공용공간을 하청 없이 직접 관리합니다. 작업 기록은 순차적으로 정리 중이며, 상담은 바로 가능합니다."
focus="안평리·매곡리·주박리 등 호법면 생활권의 계단·복도·공동현관 상태를 확인하고 관리 주기를 안내합니다."
  areaSlug="hobeop"
/>

<AreaTimeline areaName="호법면" areaSlug="hobeop" posts={posts} title="호법면 작업 일지" description="호법면 현장 사진은 정리되는 순서대로 작업 일지에 추가됩니다." emptyMessage="호법면 작업 기록은 사진 정리 후 순서대로 추가할게요. 상담과 견적 안내는 지금도 가능합니다." />
...
<p className="mt-4 text-sm text-muted-foreground">호법면 인근 건물 관리 피드백</p>
</div>
))}
</section>
...
<h2 className="mb-3 text-2xl font-extrabold text-foreground md:text-3xl">호법면 청소 관리가 필요하신가요?</h2>
