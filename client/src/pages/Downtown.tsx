  ...
  return (
    <main className="min-h-screen bg-gradient-to-b from-white via-blue-50/20 to-white">
      <section className="container max-w-6xl pt-24 pb-16 md:pt-32 md:pb-24">
        ...
        <section className="mb-12 rounded-[1.5rem] border border-blue-100 bg-white p-6 shadow-sm md:mb-16">
          <div className="mb-4 flex items-center gap-2 text-primary">
            <MapPin className="h-5 w-5" />
            <h2 className="text-xl font-extrabold text-foreground md:text-2xl">시내권 관리 가능 지역</h2>
          </div>
          <div className="flex flex-wrap gap-2">
            {serviceAreas.map((area) => (
              <span key={area} className="rounded-full bg-blue-50 px-4 py-2 text-sm font-bold text-primary">
                {area}
              </span>
            ))}
          </div>
        ...
        <section className="mb-12 md:mb-16">
          <div className="mb-5 flex items-end justify-between gap-4">
            <div>
              <h2 className="text-xl font-extrabold text-foreground md:text-2xl">시내권 작업 기록</h2>
              <p className="mt-1 text-sm text-muted-foreground">관고동·창전동·증포동 등 시내권 현장을 기준으로 업데이트됩니다.</p>
            </div>
